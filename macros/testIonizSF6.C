...
void testIonizSF6(int e=50, int i=0, const char * ion = "f")
{ 

//  retrim::CollisionReader c("data/coll/COLLISON_200keV.txt"); 
//  retrim::CollisionReader c("data/c/60torr/coll/COLLISON_150.00keV.txt"); 

  retrim::CollisionReader c(TString::Format("data/%s/20torr/coll/COLLISON_%d.00keV.%d.txt",ion, e,i)); 

  const retrim::TableReader * tables[3]; 
  tables[0] = new retrim::TableReader("data/srim/F_in_SF6_20torr.txt"); 
  tables[1] = new retrim::TableReader("data/srim/S_in_SF6_20torr.txt"); 
  tables[2] = new retrim::TableReader("data/srim/He_in_SF6_20torr.txt"); 


  TH1F * true_ion = 0; 
  TH1F * true_recoil = 0; 

  retrim::HistogramReader::readIONIZ(TString::Format("data/%s/20torr/ioniz/IONIZ_%d.00keV.%d.txt",ion,e,i),&true_ion, &true_recoil); 
...
//  t.getStats()->Draw("ElecLoss:predictedElecLoss","predictedElecLoss < 0.1","colz"); 
  ...
  retrim::HistogramReader::readPHONON(TString::Format("data/%s/20torr/phonon/PHONON%d.00keV.%d.txt",ion,e,i),&phonon_true_ion, &phonon_true_recoil); 
